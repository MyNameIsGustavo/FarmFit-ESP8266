...
int porcen;
int umidade = 22; // Define o pino 22 como Pino Analógico do sensor


int Rele = 23; // Pino Digital D23 como Relé
...
int contador = 0;
int fluxo_agua = 21; // Pino do Sensor de Fluxo
...
void umidade_solo(){
  ValAnalogIn = analogRead(umidade);
  int porcen = map(ValAnalogIn, 1023, 0, 0, 100); // Traforma o valor analógico em porcentagem

  Serial.print("Umidade: "); // Imprime o símbolo no valor
  Serial.print(porcen); // Imprime o valor em Porcentagem no monitor Serial
  Serial.println("%");

  if (porcen <= 76) { // Se a porcentagem for menor ou igual à 76%. OBS: Você pode alterar essa porcentagem
...
void enviaValores() {
  ...
  //Ler a umidade 
  mqttClient.publish(topic, "Fluxo de Agua: " + String(flowRate) + " Umidade do Solo: " + String(porcen) + "%");
